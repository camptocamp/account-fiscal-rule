...
class Tests(TransactionCase):
    ...
    def setUp(self):
        ...
        self.account_fiscal_position_test_for_rule = self.env[
        ...
        self.fiscal_position_rule = self.env[
            ...
            'fiscal_position_id':
            self.account_fiscal_position_test_for_rule.id,
            'use_purchase': True,
        })
        self.account_fiscal_position_test = self.env[
            'account.fiscal.position'
        ].create({
            'name': 'internal_purchase',
            'auto_apply': True,
        })
        self.partner_without_fiscal_position = self.env[
            'res.partner'
        ].create({
            'name': 'partner_name_without_fiscal_position',
            'supplier': True,
        })
        self.partner_with_fiscal_position = self.env[
            'res.partner'
        ].create({
            'name': 'partner_name_with_fiscal_position',
            'property_account_position_id':
            self.account_fiscal_position_test.id,
            'supplier': True,
        })
